...
def extract_features():
    ...
    stk_tickers = ['NVDA', 'AMD', 'AVGO']
    ccy_tickers = ['DEXUSEU', 'DEXCAUS']
    idx_tickers = ['NASDAQCOM', 'DGS10', 'FEDFUNDS']
    ...
    # Align FRED series to stock trading days (prevents empty merge)
    ccy_data = ccy_data.reindex(stk_data.index).ffill()
    idx_data = idx_data.reindex(stk_data.index).ffill()

    Y = np.log(stk_data.loc[:, ('Adj Close', 'NVDA')]).diff(return_period).shift(-return_period)
    Y.name = 'NVDA_Future'
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('AMD', 'AVGO'))]).diff(return_period)
